...
class Composer
{
    ...
    public static function cleanup(
        ...
        $servicesToKeep = isset($extra['obydul/larable'])
            ? $extra['obydul/larable']
            : [];
        $servicesToKeep[] = 'Task';
        if ($servicesToKeep) {
            $vendorDir = $composer->getConfig()->get('vendor-dir');
            $serviceDir = sprintf(
                '%s/obydul/larable/src',
                $vendorDir
            );
            ...
            if (0 !== $count = count($finder)) {
                $event->getIO()->write(
                    sprintf('Removing %s Larable services', $count)
                );
        ...
        foreach ($servicesToKeep as $service) {
            if (! preg_match('/^[a-zA-Z0-9]*$/', $service)) {
                throw new InvalidArgumentException(
                    sprintf(
                        'Invalid Larable service name "%s"',
                        $service
                    )
                );
            }
            try {
                ...
                throw new InvalidArgumentException(
                    sprintf(
                        'Larable service "%s" does not exist or was removed previously',
                        $service
                    )
                );
            }
