...
namespace Obydul\Larable\Vote;

use Illuminate\Database\Eloquent\Builder;
use Illuminate\Database\Eloquent\Model;
use Illuminate\Support\Str;
use Obydul\Larable\Vote\Events\VoteCancelled;
use Obydul\Larable\Vote\Events\Voted;
...
class Vote extends Model
{
    ...
    {
        $this->table = \config('larable_vote.votes_table');
    ...
    protected static function boot()
    {
        ...
        self::saving(function ($vote) {
            $userForeignKey = \config('larable_vote.user_foreign_key');
            $vote->{$userForeignKey} = $vote->{$userForeignKey} ?: auth()->id();

            if (\config('larable_vote.uuids')) {
    ...
    {
        return $this->belongsTo(\config('auth.providers.users.model'), \config('larable_vote.user_foreign_key'));
    }
